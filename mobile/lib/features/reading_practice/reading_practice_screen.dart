...
import 'dart:convert';
import '../../../core/services/reading_practice_api_service.dart';
...
class _ReadingPracticeScreenState extends State<ReadingPracticeScreen> {
  ...
  Future<void> _loadLessonData() async {
    ...
    try {
      final partsData = await ReadingPracticeApiService.getLessonParts(lessonId: widget.lessonId);
      ...
      for (var part in _parts) {
        ...
        try {
          final partDetailData = await ReadingPracticeApiService.getPartDetails(partId: partId);
  ...
  Future<void> _loadCorrectAnswers(int partId, String correctAnswerPath) async {
    try {
      final content = await ReadingPracticeApiService.fetchCorrectAnswers(correctAnswerPath);
    ...
    try {
      for (var partId in _partAnswers.keys) {
        ...
        if (answers.isNotEmpty) {
          final result = await ReadingPracticeApiService.submitPartAnswers(
  ...
  void _checkAnswer(int questionId) {
    ...
    if (questionIndex >= 0 && questionIndex < correctAnswersData.length) {
      ...
      ReadingPracticeApiService.submitAnswer(
        partId: partId, 
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(widget.readingTitle, style: const TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.w600)),
      ),
      body: _isLoading
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                Row(children: [
                 // Expanded(child: Text(widget.readingTitle, textAlign: TextAlign.center, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold), overflow: TextOverflow.fade)),
                 Text(widget.readingTitle, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF2D3142))) 
                ]),
