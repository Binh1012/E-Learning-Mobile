...
import '../../../core/services/listening_practice_api_service.dart';
...
class _ListeningPracticeScreenState extends State<ListeningPracticeScreen> {
  ...
  Future<void> _loadLessonData() async {
    ...
    try {
      final partsData = await ListeningPracticeApiService.getLessonParts(lessonId: widget.lessonId);
      ...
      for (var part in _parts) {
        ...
        try {
          final partDetailData = await ListeningPracticeApiService.getPartDetails(partId: partId);
          ...
          // Tìm audio URL từ part "Audio"
          if (partDescription == 'Audio') {
            for (var question in questions) {
              final displayOrders = question['displayOrders'] as List? ?? [];
              for (var display in displayOrders) {
                if (display['content_type'] == 'audio' && display['content_path'] != null) {
          ...
          // Xử lý questions từ part "Question"
          if (partDescription == 'Question') {
            for (var question in questions) {
              final displayOrders = question['displayOrders'] as List? ?? [];
              
              // Kiểm tra xem có content_path null không
              bool hasNullContentPath = displayOrders.isEmpty || 
                displayOrders.every((d) => d['content_path'] == null);
              
              if (hasNullContentPath && correctAnswerPath != null) {
                // Fetch correct answers để đếm số câu hỏi
                try {
                  final answersText = await ListeningPracticeApiService.fetchCorrectAnswers(correctAnswerPath);
                  final totalQuestions = answersText.trim().length;
                  
                  print('📝 Found $totalQuestions questions from correct_answer_path');
                  
                  // Tạo virtual questions theo số lượng chữ cái trong correct_answer_path
                  for (int i = 0; i < totalQuestions; i++) {
                    final virtualQuestion = {
                      'question_id': question['question_id'] * 1000 + i, // Unique ID
                      'question_number': i + 1,
                      'part_id': partId,
                      'part_description': partDescription,
                      'is_virtual': true, // Flag để biết đây là virtual question
                      'virtual_index': i, // Index trong list virtual questions
                      'displayOrders': [],
                    };
                  ...
                  // Fallback: add question như bình thường
                ...
                // Question có content_path bình thường
                final questionWithPart = Map<String, dynamic>.from(question);
                questionWithPart['part_id'] = partId;
                questionWithPart['part_description'] = partDescription;
                allQuestions.add(questionWithPart);
    ...
    try {
      for (var partId in _partAnswers.keys) {
        ...
        if (answers.isNotEmpty) {
          final result = await ListeningPracticeApiService.submitPartAnswers(
