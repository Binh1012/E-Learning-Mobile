...
class _ListeningPracticeScreenState extends State<ListeningPracticeScreen> {
  ...
  Future<void> _loadCorrectAnswers(int partId, String correctAnswerPath) async {
    try {
      final content = await ListeningPracticeApiService.fetchCorrectAnswers(correctAnswerPath);
  ...
  void _checkAnswer(int questionId) {
    ...
    if (questionIndex >= 0 && questionIndex < correctAnswersData.length) {
      ...
      ListeningPracticeApiService.submitAnswer(
        partId: partId, 
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(widget.listeningTitle, style: const TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.w600)),
      ),
      body: _isLoading
  ...
  Widget _buildTheoryDisplayContent(Map<String, dynamic> displayOrder, int currentIndex, int totalDisplays, int questionId) {
    ...
    final bodyContent = lines.length > 0 ? lines.sublist(0).join('\n').trim() : '';

    return SingleChildScrollView(
      padding: const EdgeInsets.symmetric(horizontal: 20.0),
      child: Column(
        children: [
          //Text(widget.readingTitle, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF2D3142))),
          //Text(widget.lessonDescription, style: TextStyle(fontSize: 16, color: Colors.grey[400])),
          const SizedBox(height: 24),
          Container(
            ...
            child: Column(
              children: [
                Row(children: [
                 // Expanded(child: Text(widget.listeningTitle, textAlign: TextAlign.center, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold), overflow: TextOverflow.fade)),
                 Text(widget.listeningTitle, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF2D3142))) 
                ]),
